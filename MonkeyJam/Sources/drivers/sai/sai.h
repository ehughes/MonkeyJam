...
/* Which pin mux setting to use */

#define PIN_MUX_SETTING0    0
#define PIN_MUX_SETTING1    1
#define PIN_MUX_SETTING2    2
...
typedef enum { BUS_CLOCK = 0, I2S0_MCLK, I2S1_MCLK } SAI_MASTERCLK_SEL;

/* Bit clock polarity */

typedef enum { BIT_CLOCK_ACTIVE_HIGH = 0, BIT_CLOCK_ACTIVE_LOW } SAI_BITCLK_POLARITY;

/* Bit clock direction */

typedef enum { BIT_CLOCK_EXTERNAL = 0, BIT_CLOCK_INTERNAL } SAI_BITCLK_DIR;

/* Channel enable */

typedef enum { CHANNEL_ONE_EN = 1, CHANNEL_TWO_EN = 2, CHANNEL_BOTH_EN = 3 } SAI_CHANNEL_EN;

/* MSB or LSB first */

typedef enum { LSB_TX_RX_FIRST = 0, MSB_TX_RX_FIRST } SAI_MSB_FIRST;

/* frame sync early */

typedef enum { FS_ASSERT_FIRST_BIT = 0, FS_ASSERT_EARLY } SAI_FRAME_SYNC_EARLY;

/* frame sync polarity */

typedef enum { FS_ACTIVE_HIGH = 0, FS_ACTIVE_LOW } SAI_FRAME_SYNC_POLARITY;

/* frame sync direction */

typedef enum { FS_EXTERNAL = 0, FS_INTERNAL } SAI_FRAME_SYNC_DIR;

/* master clock input */

typedef enum { SYSTEM_CLOCK = 0, OSC0ERCLK, OSC1ERCLK, MCGPLLCLK, MCGFLLCLK } SAI_MCLK_INPUT;
