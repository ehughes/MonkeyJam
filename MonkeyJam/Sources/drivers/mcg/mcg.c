...
)
{
    ...
    if
    (
        !((((MCG_S & MCG_S_CLKST_MASK) >> MCG_S_CLKST_SHIFT) == 0x3)    // check CLKS mux has selcted PLL output
...
(int crystal_val)
{
    ...
    if
    (
        !((((MCG_S & MCG_S_CLKST_MASK) >> MCG_S_CLKST_SHIFT) == 0x2)    // check CLKS mux has selcted external reference
          &&  (!(MCG_S & MCG_S_IREFST_MASK))  // check FLL ref is external ref clk
          &&  (MCG_S & MCG_S_PLLST_MASK)      // check PLLS mux has selected PLL
          &&  (!(MCG_C2 & MCG_C2_LP_MASK))
         )
    )               // check MCG_C2[LP] bit is not set
        {
...
)
{
    ...
    if(((crystal_val / prdiv_val) < 2000000) || ((crystal_val / prdiv_val) > 4000000))
        {
            return 0x43;
        }
...
)
{
    ...
    if(erefs_val)
        {
            if((crystal_val < 30000) || ((crystal_val > 40000) && (crystal_val < 3000000)) || (crystal_val > 32000000))
                {
...
(int crystal_val)
{
    ...
    if
    (
        !((((MCG_S & MCG_S_CLKST_MASK) >> MCG_S_CLKST_SHIFT) == 0x0)    // check CLKS mux has selcted FLL
          &&  (!(MCG_S & MCG_S_IREFST_MASK))  // check FLL ref is external ref clk
          &&  (!(MCG_S & MCG_S_PLLST_MASK))
         )
    )               // check PLLS mux has selected FLL
...
)
{
    ...
    if
    (
        !((((MCG_S & MCG_S_CLKST_MASK) >> MCG_S_CLKST_SHIFT) == 0x1)    // check CLKS mux has selcted int ref clk
          &&  (MCG_S & MCG_S_IREFST_MASK)     // check FLL ref is internal ref clk
          &&  (!(MCG_S & MCG_S_PLLST_MASK))   // check PLLS mux has selected FLL
          &&  (!(MCG_C2 & MCG_C2_LP_MASK))
         )
    )                       // check LP bit is clear
        {
    ...
    else
        {
            if(!irc_select)
            ...
            else
                {
                    if
                    (
                        (((MCG_C4 & MCG_C4_FCTRIM_MASK) >> MCG_C4_FCTRIM_SHIFT) == 0xF)
